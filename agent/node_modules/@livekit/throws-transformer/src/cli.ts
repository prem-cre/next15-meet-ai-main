 ...
 *
 * Usage:
 *   npx @livekit/throws-transformer 'src/*.ts'
 */
...
function main(): void {
  ...
  if (tsconfigPath) {
    ...
    if (parsedConfig.errors.length > 0) {
      const msg = ts.flattenDiagnosticMessageText(parsedConfig.errors[0].messageText, '\n');
  ...
  for (const fileName of files) {
    ...
    for (const result of results) {
      ...
      console.log(`\x1b[31m✗\x1b[0m ${relativePath}:${result.line}:${result.column}`);
